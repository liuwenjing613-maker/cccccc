...
class PreconvolutedANCDataset(Dataset):
    ...
    def __len__(self):
        """
        Original baseline:
            len(dataset) = number of paths.

        Run1 modification:
            For training only, use a virtual dataset length.
            This increases the number of random noise segments sampled per epoch.

        Testing remains deterministic and unchanged.
        """
    ...
    def __getitem__(self, idx):
        """
        Training:
            use idx % len(path_indices) so paths remain balanced even when
            samples_per_epoch is much larger than the number of paths.

        Testing:
            keep original deterministic indexing.
        """
        if self.is_train:
